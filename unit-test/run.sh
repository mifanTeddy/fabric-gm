...
serial_packages=(
    "github.com/hyperledger/fabric-gm/gossip"
)
...
packages_diff() {
    git -C "${GOPATH}/src/github.com/hyperledger/fabric" diff --no-commit-id --name-only -r "${1:-HEAD}" |
        ...
        sed 's%/[^/]*$%/%' | sort -u | \
        awk '{print "github.com/hyperledger/fabric-gm/"$1"..."}'
}
...
main() {
    ...
    local package_spec=${TEST_PKGS:-github.com/hyperledger/fabric-gm/...}

    # extra exclusions for ppc and s390x
    local arch=`uname -m`
    if [ x${arch} == xppc64le -o x${arch} == xs390x ]; then
        excluded_packages+=("github.com/hyperledger/fabric-gm/core/chaincode/platforms/java")
    fi

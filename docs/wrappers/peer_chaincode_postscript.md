...
Here are some examples of the `peer chaincode list ` command:
  ...
  * Using the `--installed` flag to list the chaincodes installed on a peer.
    ...
    Name: mycc, Version: 1.0, Path: github.com/hyperledger/fabric-gm/examples/chaincode/go/chaincode_example02, Id: 8cc2730fdafd0b28ef734eac12b29df5fc98ad98bdb1b7e0ef96265c3d893d61
  ...
  * Using the `--instantiated` in combination with the `-C` (channel ID) flag to
    ...
    Name: mycc, Version: 1.0, Path: github.com/hyperledger/fabric-gm/examples/chaincode/go/chaincode_example02, Escc: escc, Vscc: vscc
...
`ccpack.out`:
  ...
  ```
    peer chaincode package ccpack.out -n mycc -p github.com/hyperledger/fabric-gm/examples/chaincode/go/chaincode_example02 -v 1.1 -s -S
    .
    .
    .

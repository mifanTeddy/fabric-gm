...
import (
	...
	"github.com/hyperledger/fabric-gm/integration/helpers"
	"github.com/hyperledger/fabric-gm/integration/runner"
...
func (c *Components) Build(args ...string) {
	...
	cryptogen, err := gexec.Build("github.com/hyperledger/fabric-gm/common/tools/cryptogen", args...)
	Expect(err).NotTo(HaveOccurred())
	c.Paths["cryptogen"] = cryptogen

	idemixgen, err := gexec.Build("github.com/hyperledger/fabric-gm/common/tools/idemixgen", args...)
	Expect(err).NotTo(HaveOccurred())
	c.Paths["idemixgen"] = idemixgen

	configtxgen, err := gexec.Build("github.com/hyperledger/fabric-gm/common/tools/configtxgen", args...)
	Expect(err).NotTo(HaveOccurred())
	c.Paths["configtxgen"] = configtxgen

	orderer, err := gexec.Build("github.com/hyperledger/fabric-gm/orderer", args...)
	Expect(err).NotTo(HaveOccurred())
	c.Paths["orderer"] = orderer

	peer, err := gexec.Build("github.com/hyperledger/fabric-gm/peer", args...)
	Expect(err).NotTo(HaveOccurred())
	c.Paths["peer"] = peer

	discover, err := gexec.Build("github.com/hyperledger/fabric-gm/cmd/discover", args...)
	Expect(err).NotTo(HaveOccurred())
